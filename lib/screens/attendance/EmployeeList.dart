...
import 'package:ams/screens/attendance/ViewAttendanceForParticularUser.dart';
import 'package:ams/providers/AuthController.dart';

class EmployeeListScreen extends HookConsumerWidget {
  final String companyId;

  EmployeeListScreen({required this.companyId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Select an employee', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
                    ...
                    if (userId != null) {
                      if (userRoles.value.contains('hr') && userRoles.value.contains('supervisor')) {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => ViewAttendanceForParticularUser(employeeId: userId),
                          ),
                        );
                      }
                      else {
